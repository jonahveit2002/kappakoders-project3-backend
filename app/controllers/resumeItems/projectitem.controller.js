...
exports.findAll = async (req, res) => {
    ...
    try {
        const { sectionId } = req.params;
        const projectItems = await ProjectItem.findAll({ where: { section_id: sectionId} });
        res.status(200).send(projectItems);
    } catch (err) {
        res.status(500).send({ message: err.message || "Some error occurred while retrieving ProjectItems." });
    }
};
...
exports.findOne = async (req, res) => {
    try {
        ...
        if (!projectItem) {
            return res.status(404).send({ message: `ProjectItem with id=${item_id} not found.` });
        }

        res.status(200).send(projectItem);
    } catch (err) {
        res.status(500).send({ message: err.message || `Error retrieving ProjectItem with id=${item_id}` });
    }
};
...
exports.update = async (req, res) => {
    try {
        ...
        res.status(500).send({ message: err.message || `Error updating ProjectItem with id=${item_id}` });
    }
};
...
exports.delete = async (req, res) => {
    try {
        ...
        res.status(500).send({ message: err.message || `Could not delete ProjectItem with id=${item_id}` });
    }
};
...
exports.findAll = async (req, res) => {
    ...
    try {
        const { sectionId } = req.params;
        const experienceItems = await ExperienceItem.findAll({ where: { section_id: sectionId} });
        res.status(200).send(experienceItems);
    } catch (err) {
        res.status(500).send({ message: err.message || "Some error occurred while retrieving ExperienceItems." });
    }
};
...
exports.findOne = async (req, res) => {
    try {
        ...
        if (!experienceItem) {
            return res.status(404).send({ message: `ExperienceItem with id=${item_id} not found.` });
        }

        res.status(200).send(experienceItem);
    } catch (err) {
        res.status(500).send({ message: err.message || `Error retrieving ExperienceItem with id=${item_id}` });
    }
};
...
exports.update = async (req, res) => {
    try {
        ...
        res.status(500).send({ message: err.message || `Error updating ExperienceItem with id=${item_id}` });
    }
};
...
exports.delete = async (req, res) => {
    try {
        ...
        res.status(500).send({ message: err.message || `Could not delete ExperienceItem with id=${item_id}` });
    }
};
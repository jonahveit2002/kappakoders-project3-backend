...
exports.create = async (req, res) => {
    try {
        ...
        const awardItem = await AwardItem.create({ award_id, section_id: sectionId });
...
exports.findAll = async (req, res) => {
    ...
    try {
        const { sectionId } = req.params;
        const awardItems = await AwardItem.findAll({ where: { section_id: sectionId} });
        res.status(200).send(awardItems);
    } catch (err) {
        res.status(500).send({ message: err.message || "Some error occurred while retrieving AwardItems." });
    }
};
...
exports.findOne = async (req, res) => {
    try {
        ...
        if (!awardItem) {
            return res.status(404).send({ message: `AwardItem with id=${item_id} not found.` });
        }

        res.status(200).send(awardItem);
    } catch (err) {
        res.status(500).send({ message: err.message || `Error retrieving AwardItem with id=${item_id}` });
    }
};
...
exports.update = async (req, res) => {
    try {
        ...
        res.status(500).send({ message: err.message || `Error updating AwardItem with id=${item_id}` });
    }
};
...
exports.delete = async (req, res) => {
    try {
        ...
        res.status(500).send({ message: err.message || `Could not delete AwardItem with id=${item_id}` });
    }
};
...
exports.findAll = async (req, res) => {
    ...
    try {
        const { sectionId } = req.params;
        const linkItems = await LinkItem.findAll({ where: { section_id: sectionId} });
        res.status(200).send(linkItems);
    } catch (err) {
        res.status(500).send({ message: err.message || "Some error occurred while retrieving LinkItems." });
    }
};
...
exports.findOne = async (req, res) => {
    try {
        ...
        if (!linkItem) {
            return res.status(404).send({ message: `LinkItem with id=${item_id} not found.` });
        }

        res.status(200).send(linkItem);
    } catch (err) {
        res.status(500).send({ message: err.message || `Error retrieving LinkItem with id=${item_id}` });
    }
};
...
exports.update = async (req, res) => {
    try {
        ...
        res.status(500).send({ message: err.message || `Error updating LinkItem with id=${item_id}` });
    }
};
...
exports.delete = async (req, res) => {
    try {
        ...
        res.status(500).send({ message: err.message || `Could not delete LinkItem with id=${item_id}` });
    }
};
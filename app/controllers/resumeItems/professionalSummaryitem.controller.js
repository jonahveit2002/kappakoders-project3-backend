  ...
  try {
    ...
    if (!professionalSummary_id || !sectionId) {
      return res.status(400).send({
        message: "professionalSummary ID and Section ID are required!",
      });
    }

    const professionalSummaryItem = await ProfessionalSummaryItem.create({
...
exports.findAll = async (req, res) => {
  ...
  try {
    const { sectionId } = req.params;
    const professionalSummaryItems = await ProfessionalSummaryItem.findAll({
      where: { section_id: sectionId },
      include: [{ model: ProfessionalSummary, as: "professionalSummary" }],
    });
...
exports.findOne = async (req, res) => {
  try {
    ...
    const professionalSummaryItem = await ProfessionalSummaryItem.findByPk(
      item_id
    );

    if (!professionalSummaryItem) {
      return res.status(404).send({
        message: `ProfessionalSummaryItem with id=${item_id} not found.`,
      });
    }
    ...
    res.status(500).send({
      message:
        err.message ||
        `Error retrieving ProfessionalSummaryItem with id=${item_id}`,
    });
  }
};
...
exports.update = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .send({ message: "ProfessionalSummaryItem updated successfully." });
  } catch (err) {
    res.status(500).send({
      message:
        err.message ||
        `Error updating ProfessionalSummaryItem with id=${item_id}`,
    });
  }
};
...
exports.delete = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .send({ message: "ProfessionalSummaryItem deleted successfully." });
  } catch (err) {
    res.status(500).send({
      message:
        err.message ||
        `Could not delete ProfessionalSummaryItem with id=${item_id}`,
    });
  }
};
